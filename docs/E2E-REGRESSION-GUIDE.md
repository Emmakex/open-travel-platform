...
- **Prefer semantic selectors.** Use `getByRole`, stable IDs, `role`, `aria-live`, `aria-invalid`, `aria-describedby` and other programmatic relationships over fragile visual text selectors.
...
PR #116 applies the PR #115 regression pattern to Operator accessibility coverage:

- the Operator accessibility spec remains isolated under `test:accessibility-operator`;
- localized form names accept both English and Spanish;
- the Operator accessibility invariant explicitly protects those bilingual expectations.
...
- Can a stable semantic selector replace a text literal?
- Does the test environment provide exactly the persistent services/capabilities the journey requires?

If any answer is unclear, compare the change against PR #115 before merging.
