...
import { renderTravellerListPdf, travellerListFilename } from "@/lib/departure-document-pdf";
...
export async function GET(
  ...
  const pdf = await renderTravellerListPdf(input);

  return new NextResponse(Buffer.from(pdf), {
    headers: {
      "Content-Type": "application/pdf",
      "Content-Disposition": `attachment; filename="${travellerListFilename(input)}"`,
