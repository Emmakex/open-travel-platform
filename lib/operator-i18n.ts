...
export function formatOperatorMoney(value: number, currency: string, locale: TravelLocale, maximumFractionDigits = 0) {
  return new Intl.NumberFormat(localeTag(locale), { style: "currency", currency, maximumFractionDigits }).format(value);
}

export function formatOperatorDate(value: string | Date | undefined, locale: TravelLocale, withTime = false) {
  if (!value) return "—";
  return new Intl.DateTimeFormat(localeTag(locale), { dateStyle: "medium", ...(withTime ? { timeStyle: "short" as const } : {}) }).format(new Date(value));
}

export function reservationStatusLabel(value: string, locale: TravelLocale) {
  const labels: Record<string, [string, string]> = { pending: ["Pending", "Pendiente"], confirmed: ["Confirmed", "Confirmada"], cancelled: ["Cancelled", "Cancelada"] };
  const label = labels[value]; return label ? tr(locale, label[0], label[1]) : value;
}

export function staffRoleLabel(value: string, locale: TravelLocale) {
  const labels: Record<string, [string, string]> = { admin: ["Admin", "Administrador"], operator: ["Operator", "Operador"], customer: ["Customer", "Cliente"], staff: ["Staff", "Personal"] };
  const label = labels[value]; return label ? tr(locale, label[0], label[1]) : value;
}

export function accountStatusLabel(value: string, locale: TravelLocale) {
  const labels: Record<string, [string, string]> = { active: ["Active", "Activo"], disabled: ["Disabled", "Desactivado"] };
  const label = labels[value]; return label ? tr(locale, label[0], label[1]) : value;
}

export function publicationStatusLabel(value: string, locale: TravelLocale) {
  const labels: Record<string, [string, string]> = { draft: ["Draft", "Borrador"], published: ["Published", "Publicado"] };
  const label = labels[value]; return label ? tr(locale, label[0], label[1]) : value;
}

export function departureStatusLabel(value: string, locale: TravelLocale) {
  const labels: Record<string, [string, string]> = { open: ["Open", "Abierta"], closed: ["Closed", "Cerrada"], "sold-out": ["Sold out", "Agotada"] };
  const label = labels[value]; return label ? tr(locale, label[0], label[1]) : value;
}

export function focalPointLabel(value: string, locale: TravelLocale) {
  const labels: Record<string, [string, string]> = { center: ["Center", "Centro"], top: ["Top", "Arriba"], bottom: ["Bottom", "Abajo"], left: ["Left", "Izquierda"], right: ["Right", "Derecha"] };
  const label = labels[value]; return label ? tr(locale, label[0], label[1]) : value;
}
...
export function authEventLabel(value: string, locale: TravelLocale) {
  ...
  const label = labels[value]; return label ? tr(locale, label[0], label[1]) : value.replaceAll("_", " ");
}
